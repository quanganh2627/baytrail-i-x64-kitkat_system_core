...
uintptr_t rewind_pc_arch(const memory_t* memory __attribute__((unused)), uintptr_t pc) {
    /* TODO: x86 instructions are 1-16 bytes, to define exact size of previous instruction
       ...
       pc is probably enough approximation for now */
    return pc;
}
...
static bool try_get_uleb128(const memory_t* memory, uintptr_t ptr, uint32_t* out_value, uint32_t *cursor) {
    ...
    if (c * 7 > 32) {
       ALOGE("try_get_uleb128: data exceeds expected 4 bytes maximum");
       return false;
    }
    *out_value = val;
    return true;
...
static bool try_get_sleb128(const memory_t* memory, uintptr_t ptr, uint32_t* out_value, uint32_t *cursor) {
    ...
    if (c * 7 > 32) {
       ALOGE("try_get_sleb128: data exceeds expected 4 bytes maximum");
       return false;
    }
...
static uintptr_t find_fde(const memory_t* memory,
    ...
    /* Checking first word of eh_frame_hdr:
       1st byte is version
       ...
       If something is not expected here next section of reading the data should be redesigned.
    */
    ...
    /* TODO: 3rd byte can be DW_EH_PE_omit, that means no lookup table available and we should
       try to parse eh_frame instead. Not sure how often it may occur...
    */
    ...
    */
    ...
    uintptr_t start = 0;
    uintptr_t fde = 0;
    /* eh_frame_hdr + c points to lookup table here */
    while (low <= high) {
...
static bool update_state (const memory_t* memory, unwind_state_t *state,
    ...
    /* We can update more registers here if state has them. Meanwile doing minimal work here. */
    /* Getting CFA */
    uintptr_t cfa = 0;
...
static bool execute_fde(const memory_t* memory,
    ...
    */
    ...
    if (cie_info->aug_P) {
        /* Get encoding of personality routine pointer */
