...
bail:
    //  free the strings we've allocated.
    if (g_mtd_state.partitions)
        free(g_mtd_state.partitions);
    g_mtd_state.partitions = NULL;
    g_mtd_state.partitions_allocd = 0;
...
{
    if (!name) {
        fprintf(stderr, "bad args to mtd_find_partition_by_name\n");
        return NULL;
    }
    if (g_mtd_state.partitions != NULL) {
        int i;
        ...
        const char *filesystem, int read_only)
{
    ...
    if (!partition) {
        fprintf(stderr, "bad args to mtd_mount_partition\n");
        return -1;
    }
        ...
        size_t *total_size, size_t *erase_size, size_t *write_size)
{
    char mtddevname[32];
    if (!partition) {
        fprintf(stderr, "bad args to mtd_partition_info\n");
        return -1;
    }
...
MtdReadContext *mtd_read_partition(const MtdPartition *partition)
{
    ...
    if (!partition) {
        fprintf(stderr, "bad args to mtd_read_partition\n");
        return NULL;
    }
...
static int read_block(const MtdPartition *partition, int fd, char *data)
{
    ...
    if (!partition) {
        fprintf(stderr, "bad args to read_block\n");
        return -1;
    }
