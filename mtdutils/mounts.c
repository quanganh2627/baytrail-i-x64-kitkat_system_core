...
bail:
    ...
    if (g_mounts_state.volumes)
        free(g_mounts_state.volumes);
    g_mounts_state.volumes = NULL;
    g_mounts_state.volumes_allocd = 0;
    g_mounts_state.volume_count = 0;
    return -1;
}
...
{
    if (!device) {
        printf("bad args to find_mounted_volume_by_device\n");
        return NULL;
    }
    if (g_mounts_state.volumes != NULL) {
        int i;
...
{
    if (!mount_point) {
        printf("bad args to find_mounted_volume_by_mount_point\n");
        return NULL;
    }
    if (g_mounts_state.volumes != NULL) {
        int i;
...
unmount_mounted_volume(const MountedVolume *volume)
{
    ...
    int ret;
    if (!volume) {
        printf("bad args to unmount_mounted_volume\n");
        return -1;
    }
...
{
    if (!volume) {
        printf("bad args to remount_read_only\n");
        return -1;
    }
