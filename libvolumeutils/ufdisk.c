...
static int new_partition(struct block_dev *s, long size, int type)
{
    static int gap = 0;
    if (!s) {
        LOGE("bad args to new_partition\n");
        return -1;
    }
    if (type == HIDDEN_PART) {
        gap += MB_TO_LBA(size);
...
static int write_partitions(struct block_dev *dev)
{
    int i, r = 0;
    if (!dev) {
        LOGE("bad args to write_partitions\n");
        return -1;
    }
...
static int wait_device_creation_timeout(const char *device, int times)
{
    ...
    if (!device) {
        LOGE("bad args to wait_device_creation_timeout\n");
        return -1;
    }
    for (i = 0; i < times; i++) {
...
int ufdisk_create_partition(void)
{
    ...
    for (i = 0; i < num_volumes; ++i) {
        ...
        if (strcmp(v->fs_type, "ext4") == 0) {
            LOGI("formatting %s\n", v->mount_point);
            if(format_volume(v->mount_point, NULL)) {
...
void load_volume_table(char *filename) {
    ...
    if (!filename) {
        LOGE("bad args to load_volume_table\n");
        return;
    }
