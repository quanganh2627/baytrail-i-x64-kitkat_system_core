...
GGLX86Assembler::GGLX86Assembler(const sp<Assembly>& assembly)
    : X86Assembler(assembly), X86RegisterAllocator(X86Assembler::getCodegenArch()), mOptLevel(7)
{
}
...
int GGLX86Assembler::scanline(const needs_t& needs, context_t const* c)
{
    ...
    if (err != 0)
        LOGE("scanline_core failed probably due to running out of the registers: %d\n", err);
...
X86RegisterAllocator::X86RegisterAllocator(int arch) : mRegs(arch)
{
}
...
X86RegisterAllocator::RegisterFile::RegisterFile(int codegen_arch)
...
X86RegisterAllocator::RegisterFile::RegisterFile(const RegisterFile& rhs, int codegen_arch)
